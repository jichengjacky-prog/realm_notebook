...
rule rosetta_discovery_round1:
    ...
    shell:
        ...
        # The done flag must only be created after the raw_scores.csv
        ...
        _cleanup_and_exit() {{
            local exit_code=$?
            # Reconcile per-ligand .done markers on ANY exit: if a ligand
            # already has all its raw_scores.csv files, make sure its .done
            # marker exists — even when this controller exits early (e.g. an
            # inner bsub failure) or ligands were completed by lingering
            # array elements. The batch done flag is never touched here.
...
PYEOF
            ...
            ARRAY_JOB_ID=$(bsub \
                ...
                {params.python_bin} -u "$CHUNK_SCRIPT" "$BATCH_DIR" "$ROUND" $LIG_LIST \
