...
open(os.path.join(batch_dir, 'round2', lig_name + '.done_pre_exit'), 'w').close()
            ...
            for JID in "${{JOB_IDS[@]}}"; do
                ...
                if [ "$STAT" = "RUN" ] || [ "$STAT" = "PEND" ]; then
                    ...
                    if [ "$STAT" = "RUN" ]; then
                        ...
                        LIG_NAME_FROM_JOB=$(bjobs -J "$JID" -noheader 2>/dev/null | sed 's/smk_ros2_//' | tr -d ' ')
                        DONE_PRE_EXIT="$BATCH_DIR/round2/${{LIG_NAME_FROM_JOB}}.done_pre_exit"
                        ...
                        if [ -f "$DONE_PRE_EXIT" ]; then
                            ...
                            OUT_FILE="$BATCH_DIR/round2/ros2_${{LIG_NAME_FROM_JOB}}.out"
                            if [ -f "$OUT_FILE" ]; then
